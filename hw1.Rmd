---
title: 'DSApps 2021 @ TAU: Assignment 1'
...
This assignemtnt is due: 22/3 23:59
...
This dataset contains ~500 top passwrods ranked by popularity (variable `rank` where 1 is most popular). For each password we also get:
...
Combine the above two outputs in a single pipe: a table with a category's name `category`, `n` the number of passwords in it and `mean_strength`, their mean strength without `NA` values. Now you *may* use `group_by()` and also have a look at [`n()`]():
...
And that's it, you have shown you can wrangle data in the Tidyverse, in a concise, consistent and extensible manner. You also know which passowrds not to use. Good luck with the rest of the course!
